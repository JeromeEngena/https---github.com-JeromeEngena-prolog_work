...
not_belong_department(M) :- not(member_of_department(M, _)), not(student_belongs(M, _)).

not_belong_department(M, N) :- not(member_of_department(M, N)), not(student_belongs(M, N)).

course_not_cross_cutting(X) :-takes_course(M, X), takes_course(N, X), student_belongs(M, Z), student_belongs(N, W).